"""Ablation eval over track_queries params (no retraining).

Writes ablation_results.{txt,json} to runs/<RUN_NAME>/.
Currently only the baseline config is active (see ABLATIONS below).
"""
...
def _comparison_table(all_results: list) -> str:
    """Side-by-side MICRO comparison per dataset."""
...
def main(run_name: str = RUN_NAME):
    ...
    # weights are shared across ablations; only cfg changes
    ...
    for abl in ABLATIONS:
        ...
        # model reads self.cfg at runtime, so mutate it in place
