...
Usage:
    ...
    python train_rfdetr.py --model-size medium             # use RFDETRMedium (576x576)
...
DEFAULT_OUTPUT = str(ROOT / "rf-detr/runs")
...
def convert_timm_to_rfdetr(timm_state: dict) -> dict:
    """Remap timm ViT keys to RF-DETR's transformers-style DINOv2 keys, splitting fused QKV."""
    rfdetr_state = {}
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default=None,
                        help="Output directory for checkpoints/logs (default: rf-detr/runs/<name>)")
    ...
    output_dir = args.output_dir or str(ROOT / "rf-detr/runs" / run_name)
    ...
    # Angiograms are grayscale: no color augs, only geometric + brightness.
    aug_config = {
        # Geometric
    ...
    # LSJ: albumentations RandomResizedCrop clamps scale to [0,1], so do it as
    # RandomScale -> PadIfNeeded -> RandomCrop. Prepend so it runs first.
    if args.lsj:
        res = MODEL_RESOLUTIONS[args.model_size]
        # scale_limit is a delta from 1.0: effective scale = 1 + Uniform(lo, hi)
        scale_lo = args.lsj_min_scale - 1.0
        scale_hi = args.lsj_max_scale - 1.0
        aug_config = {
            "Sequential": {
                "transforms": [
                    {
