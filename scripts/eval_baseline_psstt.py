"""Test-time eval of a PSSTT run. Writes ablation_results.{txt,json} to the run dir.

Metrics match _eval_stfs_ablations so they line up against the RF-DETR-video numbers.
"""
...
def _load_model(run_dir: Path, cfg: Config) -> VideoFasterRCNN:
    # checkpoint already has trained weights; skip the COCO download (offline-safe)
...
def _predict_centre(
    ...
    """Centre-frame (boxes_xyxy, scores), rescaled to original image size."""
    ...
    # model input space -> original frame
