"""PSSTT detector (Han et al., Comput Biol Med 153 (2023) 106546).

Faster R-CNN (resnet50 fpn v2, COCO pretrained) backbone + RPN with the box
head swapped for PSSTTBoxHead: per proposal it builds K+1 shifted RoIs, pools
them from all T frames, runs a TFA transformer, and fuses to one feature.
Sampling/box-coder/loss are reused from torchvision's RoIHeads.
"""
...
# shifts as (dx, dy) fractions of box w/h: zero, up, down, left, right.
...
class TFAEncoder(nn.Module):
    """Transformer encoder over the fixed-length T*(K+1) token sequence."""
...
class PSSTTBoxHead(nn.Module):
    """Shifted RoI pooling + TFA + fusion, run per (b, t0) reference frame."""
    ...
    def __init__(self, cfg: Config):
        ...
        # fuse along the token axis (T*(K+1) -> 1), keep D
    ...
    def build_shifted_boxes(
        ...
        y1s = y1s.clamp(0, H - 1)
        y2s = y2s.clamp(0, H - 1)
        # keep positive area after clipping
    ...
    def forward(
        ...
        """Aggregate features per (b, t0) -> (sum_i S_i, out_features) in (b, t0) order.

        per_frame_features: len-T list of {fpn_level: (B, C, h_l, w_l)}.
        proposals_per_bt0: (S_i, 4) xyxy pixel proposals per (b, t0) pair.
        ref_frame_per_bt0 / batch_idx_per_bt0: the t0 / b index for each pair.
        """
        T = self.T
        K1 = self.K + 1

        # one roi_pool call per support frame over all (b, t0) at once
        ...
        for proposals in proposals_per_bt0:
            ...
            sizes.append(shifted.shape[0])

        any_level = next(iter(per_frame_features[0].values()))
        B = any_level.shape[0]

        # regroup boxes by batch element b (RoIAlign wants List[Tensor] of len B),
        # keeping an index back into the original (b, t0) order
        ...
        for i, (b, shifted) in enumerate(zip(batch_idx_per_bt0, shifted_per_bt0)):
            per_b_boxes[b].append(shifted.reshape(-1, 4))
            per_b_to_bt0[b].append(i)
            per_b_counts[b].append(shifted.shape[0])
        ...
        # boxes_per_b is concatenated per (b, t0) block, so we walk pooled with a
        # cursor and slice each block back out
        cursor = 0
        ...
        for b in range(B):
            counts = per_b_counts[b]
            bt0_idxs = per_b_to_bt0[b]
            for S_i, bt0_idx in zip(counts, bt0_idxs):
                if S_i == 0:
                    continue
                block_len = S_i * K1
                block = pooled[cursor : cursor + block_len]
                cursor += block_len
...
class VideoFasterRCNN(nn.Module):
    """End-to-end PSSTT detector: shared backbone+RPN across frames, PSSTT head
    per reference frame. Forward mirrors VideoRFDETR's signature."""
    ...
    def __init__(self, cfg: Config):
        ...
        # default num_classes=91 so the COCO checkpoint loads; head/predictor
        # get replaced below
        ...
        self.rpn = frcnn.rpn
        # we feed already-normalised fixed-size tensors, so the tv transform is a
        # no-op; kept only so RPN / RoIAlign get image_sizes via ImageList
        self.tv_transform = frcnn.transform

        # reuse proposal matcher + fg/bg sampler + box-coder from RoIHeads
        rh = frcnn.roi_heads
    ...
    # lower lr on the pretrained backbone
    ...
    def select_training_samples(
        ...
        """Mirror torchvision RoIHeads.select_training_samples for one frame.

        Returns (proposals, labels, reg_targets) as list[B], with GT boxes
        prepended to the proposals.
        """
        gt_boxes = [t["boxes"] for t in targets]
        gt_labels = [t["labels"] for t in targets]
        # prepend GT boxes to proposals (torchvision)
        proposals = [
    ...
    def _backbone_per_frame(
        ...
        """Backbone+FPN once on flattened (B*T, 3, H, W); split to per-frame
        list of dicts of (B, C, h, w)."""
    ...
    def _postprocess_detections_for_frame(
        ...
        """Postprocess detections; mirrors torchvision."""
    ...
    def forward(
        ...
        targets (training only): list[B] of list[T] of {"boxes": (N,4) xyxy px,
            "labels": (N,) int64 with bg=0}.

        Train returns the loss dict. Eval returns {'centre': list[B] of
        {'boxes','scores','labels'} (pixel coords, centre=T//2)}, plus
        'all_frames' when cfg.supervise_all_frames is set.
        """
        cfg = self.cfg
        ...
        # RPN wants targets flattened to per-image (B*T)
        if is_train:
        ...
        # proposals_flat: len B*T, each (S, 4) xyxy px

        if is_train:
            ref_frames = list(range(T)) if cfg.supervise_all_frames else [T // 2]
        else:
            # eval only needs the centre frame
            ref_frames = [T // 2]
        ...
        # head batches across all (b, t0) internally and picks b via batch_idx_per_bt0
