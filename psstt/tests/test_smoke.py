"""Smoke tests for psstt. Heavy model tests are gated behind PSSTT_HEAVY=1."""
...
def test_shift_vectors_paper_layout():
    """Shift 0 is zero-shift; the other four are up/down/left/right."""
...
def test_build_shifted_boxes_geometry():
    ...
    # up shift moves box 0 (h=100) by -h: y1 300->200, y2 200->100... -> [100,100,200,200]
    from psstt.model import SHIFT_VECTORS_DEFAULT
...
def test_build_shifted_boxes_clip_at_image_boundary():
    ...
    cfg = Config()
    head = PSSTTBoxHead(cfg)
    # top-left corner box: up/left shifts must clip, not crash
    proposals = torch.tensor([[10., 10., 50., 50.]])
    shifted = head.build_shifted_boxes(proposals, image_size=(512, 512))
    # everything stays inside [0, 511] with positive area
...
def test_single_window_overfit():
    """30 SGD steps on one synthetic window should drop the loss meaningfully."""
    from psstt.model import VideoFasterRCNN
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    cfg = Config()
    cfg.T = 3
    ...
    # first-5 vs last-5 mean, expect >=20% drop
