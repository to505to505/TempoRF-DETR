...
Sliding windows of T consecutive frames with per-frame targets (len(targets) == T).
with_teacher_frame also returns all T frames at the teacher HR resolution with the
same geometric replay, so per-frame CRRCD distillation stays spatially aligned.
"""
...
# reuse the sequence_dataset helpers so layout parsing stays in one place
...
class VideoStenosisDataset(Dataset):
    ...
    def _augment_one(
        ...
        """Build a replay on the first frame, reuse it for the rest of the window."""
    ...
    def _load_teacher_window(self, paths, saved_replay) -> torch.Tensor:
        """T frames at teacher resolution, replaying the student geom aug. Returns (T, 3, S, S)."""
        out = []
        for img_path in paths:
            img = cv2.imread(str(img_path), cv2.IMREAD_GRAYSCALE)
            if img is None:
    ...
    def _apply_temporal_dropout(self, frames: torch.Tensor) -> torch.Tensor:
        """Train-only: replace centre and +/-radius neighbours with Gaussian noise."""
        cfg = self.cfg
