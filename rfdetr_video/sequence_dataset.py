...
Sliding windows of T consecutive frames -> normalised tensors + targets
in RF-DETR format (normalised cxcywh boxes).
"""
...
def build_geometric_augmentation() -> ReplayCompose:
    """Geometric-only augmentation, so the same replay can be applied to
    both the student frames and the HR teacher centre frame (distillation)."""
    return ReplayCompose(
        [
            A.HorizontalFlip(p=0.5),
            A.Rotate(limit=20, border_mode=cv2.BORDER_CONSTANT, p=0.3),
...
class TemporalStenosisDataset(Dataset):
    ...
    def __init__(
        ...
        # Paired-window index: (idx_A, idx_B) where B is A shifted by
        # consistency_offset frames in the same sequence. Skip short/padded
        # sequences so both items have a real neighbour.
        ...
        if split == "train":
            if with_teacher_frame:
                # Split pipelines: teacher shares the geometric transform but
                # gets no photometric perturbation.
    ...
    def _load_window(
        ...
        """Load one window of T frames + targets. Pass saved_replay to reuse a
        previous window's augmentation (paired-window overlap, teacher frame).

        Returns (frames (T,3,H,W), targets list of {boxes,labels}, replay).
        """
        images = []
        targets = []
    ...
    def _load_teacher_centre(self, centre_path: Path, saved_replay) -> torch.Tensor:
        """HR teacher centre frame, transformed with the student window's replay."""
    ...
    def _apply_temporal_dropout(self, frames: torch.Tensor) -> torch.Tensor:
        """Replace selected frames with Gaussian noise. Train-only. Centre is
        the primary target; each neighbour within +/-radius is masked
        independently. Modifies frames in-place."""
        cfg = self.cfg
    ...
    def __getitem__(self, idx: int):
        ...
        if self.with_paired_window:
            ...
            # A creates the replay, B reuses it so overlapping frames match.
