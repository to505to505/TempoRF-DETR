...
Runs a video window as B*T frames through the RF-DETR backbone/transformer,
then reshapes predictions back to (B, T, Q, *). Optional ETF, post-net,
prompt bank, and teacher/student query hooks for distillation.
"""
...
def _param_group_for(name: str) -> str:
    """backbone / new / pretrained LR group for a param name."""
...
class VideoRFDETR(nn.Module):
    def __init__(self, cfg: Config):
        ...
        if cfg.freeze_decoder:
            ...
            for param in self.bbox_embed.parameters():
                param.requires_grad = False
        # Non-full adapt modes must not touch the 2-D detector's query priors.
        ...
        if cfg.prompt_enabled:
            ...
            # backbone projector outputs hidden_dim channels, so feat_channels == D
        ...
        def _inject_pre_hook(_module, args, kwargs):
            ...
            if "tgt_add" in injected:
                # Prompt-tuning: add prompts to first N tgt rows, leave refpoints alone.
                existing_tgt = args[0]
    ...
    def forward(
        ...
        """Student mode returns (B,T,Q,*); teacher/general return (B*T,Q,*) for KD."""
        del targets
        ...
        if hs is not None:
            ...
            if (
                ...
                # Refine final hidden state, then re-run heads with the same refpoints.
                ...
                # aux comes from frozen intermediate layers, drop it
    ...
    def get_param_groups(self):
        ...
        # postnet/prompt freeze the whole detector, so only "new" is trainable;
        # in full mode an empty pretrained bucket means a bad prefix match.
