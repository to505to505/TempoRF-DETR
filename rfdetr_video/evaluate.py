"""Evaluation for Video RF-DETR. Reports centre-frame and per-frame mAP."""
...
from rfdetr_video.sequence_eval import (
    evaluate_map,
    f1_confidence_sweep,
)
...
def evaluate(model, loader, criterion, postprocess, cfg: Config, device):
    ...
    Headline metrics (no prefix) cover centre frames of unpadded windows only,
    for a fair comparison with single-frame detectors. ``all/`` metrics cover
    all T positions with no filtering.
    """
    model.eval()
    criterion.eval()
    ...
    for images, targets_list, fnames in loader:
        ...
        # SetCriterion wants a flat list of length B*T.
        targets_flat = []
        ...
        with autocast(enabled=cfg.amp):
            ...
            # (B, T, Q, *) -> (B*T, Q, *) to match the flat targets.
            pred_flat = {
        ...
        for b in range(B):
            for t in range(T):
                ...
                if t == centre:
                    # Padding replicates the last filename, so distinct filenames == unpadded window.
