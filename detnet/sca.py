"""Sequence Consistency Alignment: eval-time post-processing over a T-frame window.

Clusters detections across frames (IoU, with an SSIM fallback for near-misses),
drops clusters present in fewer than T_frame frames as false positives, then
linearly interpolates box corners over missing frames. No learnable params.
We skip the paper's centerline-projection step (centerline extraction is out of
scope) and just interpolate the corners.
"""
...
def _resize_to(arr: np.ndarray, hw: Tuple[int, int]) -> np.ndarray:
    """Bilinear resize, no SciPy/OpenCV dependency."""
    H, W = arr.shape[:2]
    th, tw = hw
    if H == th and W == tw:
        return arr
...
def patch_ssim(img_a: np.ndarray, box_a: np.ndarray,
    ...
    """Luminance-only SSIM between two patches, resized to a common size."""
    ...
    # normalise to [0, 1] if patches are 0..255
...
def apply_sca(
    ...
    """Run SCA on one window's detections; returns a length-T list of filtered
    and interpolation-completed FrameDetections."""
    T = len(per_frame)
    if T == 0:
        return per_frame

    # one node per (frame_idx, det_idx)
    ...
    # Union-Find. Only merge across adjacent frames; non-adjacent detections
    # can still join the same cluster transitively.
    ...
    # drop clusters seen in fewer than t_frame frames
    ...
    # surviving boxes plus interpolated ones for gaps inside kept clusters
    ...
    for cluster_nodes in kept_clusters:
        ...
        # mean-of-corners box per frame, used only as the interpolation anchor;
        # real detections are emitted as-is
        ...
        for t_missing in range(present_ts[0], present_ts[-1] + 1):
            ...
            # interpolated boxes get the median cluster score
