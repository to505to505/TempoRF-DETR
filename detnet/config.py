"""Config for Stenosis-DetNet. Shares the scaffolding fields with psstt.config;
the DetNet-specific bits are the SFF and SCA hyper-parameters.
"""
...
class Config:
    ...
    roi_size: int = 7                   # 256x7x7 candidate-box feature (Table 1)
    sff_token_dim: int = 512            # D
    sff_heads: int = 8
    sff_dropout: float = 0.0
    sff_use_residual_concat: bool = True
    ...
    # SCA needs per-frame detections from every frame, so supervise all of them.
    # False = supervise only the centre frame (memory fallback).
