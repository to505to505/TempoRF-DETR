...
def test_sca_interpolates_missing_inner_frame():
    ...
    T = 5
    per_frame = []
    # present everywhere except frame 2
    for t in range(T):
        if t == 2:
    ...
    # adjacent-only matching: {0,1} and {3,4} stay disjoint, both below t_frame=3
...
def test_sca_no_interpolation_when_disabled():
    """A 5-of-5 cluster is kept even with interpolation off."""
    T = 5
    per_frame = []
    for t in range(T):
        boxes = np.array([[100., 100., 200., 200.]], dtype=np.float32)
